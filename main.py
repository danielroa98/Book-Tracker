...
if TEST is not True:
    ...
    # Initialize session state for login status using Supabase
    test_auth.init_session()

    # Login or register forms using Supabase
    if not test_auth.is_logged_in():
        st.title("Book Tracker 📚")
        ...
        if st.session_state.get("register", False):
            ...
            if st.button("Register"):
                if (
                    ...
                    message = test_auth.register_user(
                        reg_email,
                        reg_password,
                        reg_first_name,
                        reg_last_name,
            ...
            # Login form using Supabase
            st.header("Login")
            email = st.text_input("Email")
            password = st.text_input("Password", type="password")
            if st.button("Login"):
                if test_auth.login(email, password):
                    st.session_state["logged_in"] = True
